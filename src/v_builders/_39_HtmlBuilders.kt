...
fun renderProductTable(): String {
    return html {
        table {
            ...
            for (indexed in products.withIndex()) {
                val product = indexed.value
                tr(color = getTitleColor()) {
                    td(color = getCellColor(indexed.index, 0)) {
                        text(product.description)
                    }
                    td(color = getCellColor(indexed.index, 1)) {
                        text(product.price)
                    }
                    td(color = getCellColor(indexed.index, 2)) {
